...
public class id0012_EditMenuPerson extends Command {
    User user;
    ...
    public boolean execute() throws TelegramApiException {
        ...
        if (hasContact()) {
            ...
            if (menuRepo.findAll().size() == 1) {
                return COMEBACK;
            }
            int delAdminId = getDelAdminId(updateMessageText);
            if (menuRepo.findById(delAdminId) != null) {
                menuRepo.delete(menuRepo.findById(delAdminId));
    ...
    private boolean saveAdmin(String phone) throws TelegramApiException {
        ...
        if (menuRepo.findByChatId(newAdmin.getChatId()) != null) {
            deleteAlreadyAdminMessId();
            alreadyAdminMessId = sendMessage(46);
            return COMEBACK;
        }
        Menu admin = new Menu();
        admin.setChatId(usersRepo.findByPhone(phone).getChatId());
        admin.setName(newAdmin.getFullName());
        menuRepo.save(admin);
        editMessage(getListAdmins(), chatId, messId);
        return COMEBACK;
    }
    ...
    private String getListAdmins() {
        ...
        List<Menu> adminList = menuRepo.findAll();
        for (Menu admin : adminList) {
            try {
                admins.append(admin.getName()).append(" ");
                if (adminList.size() > 1)
